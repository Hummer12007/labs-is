...
public class BFSLogic : MonoBehaviour, IPacmanLogic
{
    ...
    public List<string> FindWay(Transform target, Transform pacman)
    {
        ...
        Queue<(List<string>, string)> nodes = new Queue<(List<string>, string)>();
        nodes.Enqueue((new List<string>(), start));

        while(nodes.Count > 0)
        {
            (List<string>, string) current = nodes.Dequeue();
            ...
            if (neighbours.ContainsKey("Up")){
                ...
                tmp.Add("Up");
                nodes.Enqueue((tmp, neighbours.GetValueOrDefault("Up")));
            }
            if (neighbours.ContainsKey("Down")){
                List<string> tmp = new List<string>(path);
                tmp.Add("Down");
                nodes.Enqueue((tmp, neighbours.GetValueOrDefault("Down")));
            }
            if (neighbours.ContainsKey("Left")){
                List<string> tmp = new List<string>(path);
                tmp.Add("Left");
                nodes.Enqueue((tmp, neighbours.GetValueOrDefault("Left")));
            }
            if (neighbours.ContainsKey("Right")){
                List<string> tmp = new List<string>(path);
                tmp.Add("Right");
                nodes.Enqueue((tmp, neighbours.GetValueOrDefault("Right")));
            }
        }
        return null;
    }
