...
def getMap():
    map = [[0] * thisLevel.lvlWidth for _ in range(thisLevel.lvlHeight)]
    for i in range(thisLevel.lvlHeight):
        for j in range(thisLevel.lvlWidth):
            tile = thisLevel.GetMapTile(i, j)
            if tile == 1 or tile >= 100 and tile < 200:
                map[i][j] = 1 # wall
            elif tile == 2 or tile == 3:
                map[i][j] = 2 # pellet
    for ghost in ghosts.values():
        map[ghost.nearestRow][ghost.nearestCol] = 3 # ghosts
    map[player.nearestRow][player.nearestCol] = 4 # pacman
    return map

def getValidMoves(map, ii, jj):
    for a in range(4):
        i = ii + around[a][0]
        j = jj + around[a][1]
        if i >= 0 and i < thisLevel.lvlHeight and j >= 0 and j < thisLevel.lvlWidth \
...
def move(map):
    ...
    if goal is not None:
        ...
        if path is not None:
            return path[0]
    return next(getValidMoves(map, player.nearestRow, player.nearestCol))

def getDirection(map, i, j):
    if i < player.nearestRow:
        dir =  (0, -1)
    elif i > player.nearestRow:
        dir = (0, 1)
    elif j < player.nearestCol:
        dir = (-1, 0)
    elif j > player.nearestCol:
        dir = (1, 0)

    perfectY = player.nearestRow * 16
    perfectX = player.nearestCol * 16

    if perfectX > player.x and dir != (-1, 0):
        return (1, 0)
    elif perfectX < player.x and dir != (1, 0):
        return (-1, 0)
    elif perfectY > player.y and dir != (0, -1):
        return (0, 1)
    elif perfectY < player.y and dir != (0, 1):
        return (0, -1)
...
def CheckInputs(): 
    ...
    if thisGame.mode == 1:
        map = getMap()
        x, y = getDirection(map, *move(map))
        player.velX = x * player.speed
        player.velY = y * player.speed
