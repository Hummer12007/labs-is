...
int get_estimate(int n, int m, point s, point t)
{
    return min(abs(s.x - t.x), n - abs(s.x - t.x)) + min(abs(s.y - t.y), m - abs(s.y - t.y));
}
...
std::vector<point> AStar(Maze& maze, point s, point t, std::vector<point>& visited)
{
    ...
    while (true)
    {
        ...
        for (point dp : moves)
        {
            ...
            {
                distance[p1]=d+1;
                prior_queue.insert({d+1+get_estimate(n, m, p1, t), p1});
                parent[p1] = p;
            }
        }
    }
